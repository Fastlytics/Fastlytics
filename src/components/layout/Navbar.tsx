...
const Navbar = () => {
  ...
  const { loading, user } = useAuth(); // Keep for compatibility
  ...
  // Expanded Nav Items
  const navItems = [
    { name: 'Dashboard', href: '/dashboard', icon: <DashboardSquare01Icon size={18} /> },
    { name: 'Races', href: '/races', icon: <Flag01Icon size={18} /> },
    { name: 'Drivers', href: '/standings/drivers', icon: <UserGroupIcon size={18} /> },
    { name: 'Teams', href: '/standings/teams', icon: <UserMultiple02Icon size={18} /> },
  ];
              ...
              {user ? (
                <Link to="/dashboard">
                  <Button className="bg-red-600 hover:bg-red-700 text-white font-medium px-6">
                    Launch App <ArrowRight01Icon className="ml-2 h-4 w-4" />
                  </Button>
                </Link>
              ) : (
                <Link to="/auth">
