...
const TireStrategy: React.FC<TireStrategyProps> = ({
  ...
  const renderStrategyOverview = () => {
        ...
        <div className="space-y-3 p-5 animate-pulse">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="flex items-center gap-4">
              <div className="w-14 h-7 bg-neutral-800/60 rounded-md" />
              <div className="flex-1 h-7 bg-neutral-800/40 rounded-md" />
            </div>
          ))}
        </div>
      );
    ...
    if (isErrorStrategy) {
      return (
        <div className="w-full min-h-[200px] bg-red-500/5 border border-red-500/20 rounded-xl flex flex-col items-center justify-center text-red-400 p-6 mt-4">
          <AlertCircleIcon className="w-6 h-6 mb-3 opacity-60" />
          <p className="text-sm font-semibold uppercase tracking-wider">Error loading strategy</p>
          <p className="text-xs text-red-400/60 mt-1.5 font-mono">
    ...
    if (!strategyData || strategyData.length === 0) {
      return (
        <div className="w-full min-h-[200px] bg-neutral-900/20 border border-neutral-800/50 rounded-xl flex items-center justify-center text-neutral-500 p-6 mt-4">
          <span className="text-xs font-mono uppercase tracking-widest">No tire strategy data found</span>
        </div>
      );
    }
    ...
    return (
      <TooltipProvider delayDuration={0}>
        <div className="pt-2">
          {/* Legend */}
          <div className="flex flex-wrap justify-center gap-x-6 gap-y-2 mb-6 pb-4 border-b border-neutral-800/30">
              ...
              ([compound, colorClass]) =>
                compound !== 'UNKNOWN' && (
                  <div key={compound} className="flex items-center gap-2">
                    <span
                      className={cn(
                        'w-2.5 h-2.5 rounded-full ring-1 ring-white/10',
                        colorClass
                      )}
                    ></span>
          ...
          {/* Chart */}
          <div className="space-y-2.5 relative px-1">
            {/* Grid Lines */}
            <div className="absolute inset-0 flex pointer-events-none ml-20">
              {[0, 0.25, 0.5, 0.75, 1].map((tick) => (
                <div
                  key={tick}
                  className="h-full border-l border-neutral-800/20 absolute"
                  style={{ left: `${tick * 100}%` }}
                >
                  <span className="absolute -top-5 -translate-x-1/2 text-[10px] font-mono text-neutral-600">
            ...
            {strategyData.map((driverData) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    'flex items-center gap-3 group relative z-10 transition-all duration-300 cursor-pointer py-1.5 px-2 rounded-lg',
                    ...
                    favoriteDriver &&
                      ...
                      !focusedDriver
                      ? 'bg-red-500/8 border border-red-500/30 shadow-[0_0_20px_rgba(220,38,38,0.1)]'
                ...
                >
                  ...
                  <div className="flex-grow h-7 bg-neutral-900/30 relative flex rounded-md overflow-hidden ring-1 ring-neutral-800/60 group-hover:ring-neutral-600/80 transition-all">
                    {driverData.stints.map((stint, index) => {
                      ...
                      return (
                        <Tooltip key={index}>
                          ...
                          <TooltipContent
                            side="top"
                            className="bg-neutral-950/95 backdrop-blur-xl border border-neutral-700/40 text-white p-3.5 rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.5)]"
                          >
  ...
  return (
    <div
      ref={exportRef}
      className={cn('bg-neutral-950/80 border border-neutral-800/60 rounded-xl overflow-hidden', className)}
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-800/40">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-sm font-semibold uppercase tracking-[0.12em] text-neutral-300">Tire Strategy</h3>
          </div>
          <div className="flex items-center gap-3">
            <ChartExportMenu
              onExport={(format) =>
        ...
        <TabsContent
          value="overview"
          className="mt-0 p-5 animate-in fade-in slide-in-from-bottom-2 duration-500"
        >
